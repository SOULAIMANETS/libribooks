...
import allBooks from '@/lib/data.json';
...
const categories = ['All', ...Array.from(new Set((allBooks as Book[]).map((book) => book.category)))];
const faqPageData = pages.find(p => p.slug === 'faq');
const faqItems = faqPageData?.structuredContent || [];
...
export default function Home() {
  ...
  const featuredBooks = (allBooks as Book[]).filter(book => book.featured);

  const filteredBooks = (allBooks as Book[]).filter((book) => {
