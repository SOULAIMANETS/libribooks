      ...
      `).join('')}
       ${(articles as Article[]).map(article => `
        <url>
            ...
            <lastmod>${article.date || today}</lastmod> {/* Use today as fallback if date is missing */}
            <image:image>
                <image:loc>${article.coverImage}</image:loc>
                <image:title>${article.title.replace(/&/g, '&')}</image:title>
                <image:caption>${article.excerpt ? article.excerpt.replace(/&/g, '&').substring(0, 1024) : ''}</image:caption> {/* Handle potentially undefined excerpt */}
            </image:image>
        </url>
      `).join('')}
    </urlset>
