...
export default function CategoriesDashboardPage() {
    ...
    const addCategory = () => {
        ...
        const newCategory = { id: Math.max(...data.map(c => c.id), 0) + 1, name: newCategoryName };
    ...
    const deleteCategory = (categoryId: number) => {
        const categoryToDelete = data.find(c => c.id === categoryId);
    ...
    const handleUpdate = () => {
        ...
        setData(data.map(cat => cat.id === editingCategory.id ? {...cat, name: updatedName} : cat));
        toast({
