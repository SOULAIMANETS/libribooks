...
export type PopupAdFormValues = z.infer<typeof formSchema>;
...
interface PopupAdFormProps {
  ...
  onSubmit: (data: PopupAdFormValues) => void;
  onSuccess?: () => void;
}
...
export function PopupAdForm({ initialData, onSubmit, onSuccess }: PopupAdFormProps) {
   ...
   React.useEffect(() => {
    if (initialData) {
      formMethods.reset({
        ...
        content: initialData.content ?? defaultFormValues.content,
        displayDelay: initialData.display_delay ?? defaultFormValues.displayDelay,
        displayDuration: initialData.display_duration ?? defaultFormValues.displayDuration,
        isActive: initialData.is_active ?? defaultFormValues.isActive,
      });
    } else {
      formMethods.reset(defaultFormValues);
    }
  }, [initialData, formMethods, defaultFormValues]);


  const handleSubmit = React.useCallback((values: PopupAdFormValues) => {
    onSubmit(values);
  ...
  }, [initialData, onSubmit, onSuccess, formMethods, toast]);
